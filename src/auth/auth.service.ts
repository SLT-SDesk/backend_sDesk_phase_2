      ...
      if (id_token) {
        ...
        if (shouldCallErp) {
          ...
          try {
            ...
            console.error('ERP Service call threw an error:', erpError);
          }

          if (!employee) {
            console.warn(`Warning: Employee not found in ERP for ${serviceNum}, falling back to token details.`);
          } else {
        ...
        if (!role) {
          console.error(`Database Error: No role assigned for service number ${finalServiceNum}`);
          throw new UnauthorizedException(
